...
export async function GET() {
  ...
  const ranked = (result.results as Record<string, unknown>[]).map((row) => {
    ...
    const tax = price == null ? null : Math.round(price * .06); const acquisition = price == null || tax == null ? null : price + tax; const fees = resale == null ? null : Math.round(resale * .1325);
    ...
    const baseConfidence = Number(row.confidence ?? 0); const agePenalty = ageMinutes == null ? 100 : Math.min(35, Math.floor(ageMinutes / 180) * 3); const confidence = Math.max(0, Math.min(100, baseConfidence - agePenalty - (refreshFailed ? 12 : 0)));
    const trustedFresh = dataState === "LIVE" || dataState === "VERIFIED";
    ...
    return { ...row, error: row.last_error, checked_at: row.price_checked_at, status, data_state: dataState, confidence_score: confidence, price_source: price ? (baseConfidence >= 90 ? "Retailer product page" : "Retailer listing fallback") : "No successful source", discount_percent: discount, tax_cents: tax, fees_cents: fees, acquisition_cents: acquisition, resale_comp_cents: resale, resale_source: resale ? "Curated resale target" : "No verified resale comp", reference_source: hasReference ? (/founders edition/i.test(String(row.title)) ? "Manufacturer MSRP" : "Retailer reference") : "No verified MSRP", net_profit_cents: net, roi_percent: roi, deal_score: score, refresh_failed: refreshFailed, age_minutes: ageMinutes };
